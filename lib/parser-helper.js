...
(function() {
  var AST, ArgsArray, argsArray, array, binding, block, bool, catchAST, define, export_, finallyAST, funcDecl, funcall, ifAST, import_, letTask, let_, loglet, member, nullAST, number, object, operator, param, string, symbol, taskDecl, taskcall, throwAST, tryAST;
  ...
  funcDecl = function(name, params, body, returns) {
    ...
    return AST.make('procedure', name, params, body, returns);
  };
  ...
  taskDecl = function(name, params, body, returns) {
    ...
    return AST.make('task', name, params, body, returns);
  };

  funcall = function(proc, args) {
    return AST.make('funcall', proc, args);
  };
  ...
  import_ = function(module, defs) {
    if (defs == null) {
      defs = [];
    }
    return AST["import"](module, defs);
  };

  export_ = function(spec) {
    return AST["export"](spec);
  };

  binding = function(spec, as) {
    if (as == null) {
      as = null;
    }
    return AST.binding(spec, as);
  };

  let_ = function(id, defines, body) {
    var args, def, params;
    params = (function() {
    ...
    args = (function() {
      ...
      for (_i = 0, _len = defines.length; _i < _len; _i++) {
        def = defines[_i];
        _results.push(def.value);
      }
      return _results;
    })();
    return AST.funcall(AST.procedure(id, params, body), args);
  };

  letTask = function(id, defines, body) {
    var args, def, params;
    params = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = defines.length; _i < _len; _i++) {
        def = defines[_i];
        _results.push(AST.param(def.name));
      }
      return _results;
    })();
    args = (function() {
    ...
    return AST.taskcall(AST.task(id, params, body), args);
  };
