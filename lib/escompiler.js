...
(function() {
  var AST, ESCompiler, Environment, escodegen,
    __slice = [].slice;
  ...
  ESCompiler = (function() {
    ...
    ESCompiler.prototype.compile = function(ast) {
      var node;
      node = this.run(ast, Environment.make({
        newSym: true
      }));
    ...
    ESCompiler.prototype._symbol = function(ast, env) {
      var ref;
      ref = env.alias(ast);
      return this.identifier(ref.name.value);
    };
    ...
    ESCompiler.prototype._define = function(ast, env) {
      ...
      value = this.funcall(this.member(this.run(AST.moduleID, env), this.identifier('define')), [name, this.run(ast.value, env.pushEnv())]);
      id = (function() {
        switch (ast.name.type()) {
          case 'ref':
