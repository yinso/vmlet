...
(function() {
  ...
  T.register('assign', function($a) {
    var _ref;
    if (_ref = $a.value.type(), __indexOf.call('block', _ref) >= 0) {
  ...
  }, tr.trace('proc.trans', function($p, $name, $params, $body, $returns) {
    var body;
    body = T.transform(AST["return"]($body));
    return AST.procedure($name, $params, body, $returns);
  }));
  ...
  T.register('task', function($p) {
    ...
    var body;
    body = T.transform(AST["return"]($body));
    return AST.task($name, $params, body, $returns);
  });
