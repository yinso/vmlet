...
(function() {
  var AST, LineBuffer, compile, compileArray, compileAssign, compileBinary, compileBlock, compileCatch, compileDefine, compileFinally, compileFuncall, compileIf, compileLocal, compileMember, compileNull, compileObject, compileParam, compileProcedure, compileProxyVal, compileRef, compileReturn, compileScalar, compileSymbol, compileThrow, compileTry, errorlet, get, loglet, override, register, types, _compile, _compileOne;
  ...
  compileBlock = function(anf, buffer, level) {
  ...
  compileScalar = function(ast, buffer, level) {
    return buffer.push(JSON.stringify(ast.value));
  };
  ...
  compileProcedure = function(ast, buffer, level) {
    ...
    if (ast.name) {
      if (ast.name instanceof AST && ast.name.type() === 'ref') {
