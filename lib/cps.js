...
(function() {
  var AST, TR, combine, concat, cps, errorlet, get, loglet, makeCallback, makeCpsDef, normalize, override, register, types, util, _assign, _block, _catch, _cpsOne, _define, _if, _local, _makeErrorHandler, _return, _scalar, _task, _taskcall, _throw, _toplevel, _try;
  ...
  */
  ...
  _task = function(ast, contAST, cbAST) {
    var body, params;
    body = normalize(ast.body);
    params = [].concat(ast.params).concat(ast.callbackParam);
    body = body.items[0].type() === 'try' ? _cpsOne(body, cbAST, cbAST) : AST["try"](_cpsOne(body, cbAST, cbAST), [AST["catch"](ast.errorParam, AST.block([AST["return"](AST.funcall(cbAST, [ast.errorParam.ref()]))]))], null);
    return AST.procedure(ast.name, params, AST.block([body]));
  };

  register(AST.get('task'), _task);

  _block = function(anf, contAST, cbAST) {
    var i, _i, _ref;
  ...
  _taskcall = function(ast, contAST, cbAST) {
    var args;
  ...
  _local = function(ast, contAST, cbAST) {
    if (ast.isAsync()) {
  ...
  _assign = function(ast, contAST, cbAST) {
    var head;
    if (ast.isAsync()) {
      return _cpsOne(ast.value, makeCallback(contAST, cbAST, AST.param(ast.name)), cbAST);
    } else {
  ...
  _return = function(ast, contAST, cbAST) {
    var val;
    val = ast.value;
    if (val.type() === 'taskcall') {
      return _taskcall(val, contAST, cbAST);
    } else {
  ...
  _throw = function(ast, contAST, cbAST) {
  ...
  _makeErrorHandler = function(catchExp, finallyExp, cbAST, name) {
    ...
    errorBody = finallyExp ? AST["try"](_cpsOne(finallyExp.body, catchBody, catchBody), [AST["catch"](errParam, AST.block([okReturnExp]))]) : catchBody;
  ...
  _try = function(ast, contAST, cbAST) {
  ...
  _catch = function(ast, contAST, cbAST) {
    var body;
