...
(function() {
  var AST, TR, combine, concat, cps, cpsAssign, cpsBlock, cpsCatch, cpsDefine, cpsIf, cpsLocal, cpsReturn, cpsScalar, cpsTask, cpsTaskcall, cpsThrow, cpsTry, errorlet, get, loglet, makeCallback, makeCpsDef, normalize, override, register, types, util, _cpsOne, _makeErrorHandler;
  ...
  */
  ...
  cps = function(anf, contAST, cbAST) {
    ...
    ast = AST.task(void 0, [AST.param(AST.symbol('_rt'))], anf);
  ...
  cpsTask = function(ast, contAST) {
    var body, cbAST, errParam, params;
    ...
    body = body.items[0].type() === 'try' ? _cpsOne(body, cbAST, cbAST.name) : AST["try"](_cpsOne(body, cbAST, cbAST.name), [AST["catch"](errParam, AST.block([AST["return"](AST.funcall(cbAST.name, [errParam.name]))]))], null);
  ...
  cpsTry = function(ast, contAST, cbAST) {
    var bodyAST, catchExp, errParam, errorAST, errorParam, name, tryBody;
    name = AST.symbol('__handleError', 1);
    console.log('--cpsTry', ast.catches[0], ast["finally"]);
    catchExp = ast.catches.length > 0 ? ast.catches[0] : (errParam = AST.param(AST.symbol('e')), AST["catch"](errParam, AST.block([AST["throw"](errParam.name)])));
