...
(function() {
  var ANF, AST, combine, concat, cps, cpsBlock, cpsCatch, cpsDefine, cpsIf, cpsReturn, cpsScalar, cpsTask, cpsTaskcall, cpsTempvar, cpsThrow, cpsTry, errorlet, get, loglet, makeCallback, makeCpsDef, makeErrorHandler, normalize, override, register, types, util, _cpsOne;
  ...
  */
  ...
  cps = function(anf, contAST, cbAST) {
    var ast;
    if (contAST == null) {
      contAST = AST.make('symbol', '_done');
    }
    if (cbAST == null) {
      cbAST = contAST;
    }
    ast = AST.make('task', void 0, [AST.make('param', '_rt')], anf);
  ...
  cpsTaskcall = function(ast, contAST, cbAST) {
    ...
    return AST.make('return', AST.make('funcall', ast.funcall, args));
  };
  ...
  cpsTask = function(ast, contAST, cbAST) {
    ...
    return AST.make('procedure', ast.name, params, _cpsOne(body, cbAST, cbAST));
  };
  ...
  combine = function(ast, contAST) {
    if (!contAST) {
      ...
      return new ANF.ANF([ast, contAST]);
    }
  };
  ...
  concat = function(ast1, ast2) {
    ...
    if (is1anf) {
      return new ANF.ANF(ast1.items.concat(is2anf ? ast2.items : [ast2]));
    } else {
      return new ANF.ANF([ast1].concat(is2anf ? ast2.items : [ast2]));
    }
  };
  ...
  normalize = function(ast) {
    if (ANF.isANF(ast)) {
      ...
      return new ANF.ANF([ast]);
    }
  };

  makeCallback = function(contAST, cbAST, resParam) {
    if (resParam == null) {
      resParam = AST.make('param', 'res');
    }
    if (contAST === cbAST) {
      return contAST;
    } else {
      return AST.make('procedure', void 0, [AST.make('param', 'err'), resParam], AST.make('if', AST.make('symbol', 'err'), AST.make('return', AST.make('funcall', cbAST, [AST.make('symbol', 'err')])), contAST));
    }
  };
  ...
  makeCpsDef = function(type) {
    return function(ast, contAST, cbAST) {
      ...
      if (ast.isAsync()) {
        return _cpsOne(ast.val, makeCallback(contAST, cbAST, AST.make('param', ast.name)), cbAST);
      } else {
        head = AST.make(type, ast.name, _cpsOne(ast.val, null, null));
  ...
  cpsReturn = function(ast, contAST, cbAST) {
    ...
    if (val.type() === 'taskcall') {
      ...
      return AST.make('return', AST.make('funcall', cbAST, [AST.make('null'), _cpsOne(val, null, null)]));
    }
  };
  ...
  cpsIf = function(ast, contAST, cbAST) {
    ...
    return AST.make('if', ast["if"], _cpsOne(ast.then, contAST, cbAST), _cpsOne(ast["else"], contAST, cbAST));
  };
  ...
  cpsThrow = function(ast, contAST, cbAST) {
    return AST.make('return', AST.make('funcall', cbAST, [ast.val]));
  };
  ...
  makeErrorHandler = function(catchExp, finallyExp, cbAST, name) {
    ...
    return AST.make('tempvar', name, AST.make('procedure', void 0, [catchExp.param], body));
  };
