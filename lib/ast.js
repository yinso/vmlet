...
(function() {
  var ARRAY, ASSIGN, AST, BINARY, BINDING, BLOCK, BOOL, CASE, CATCH, CONTINUE, DEFAULTCASE, DEFINE, EXPORT, FINALLY, FUNCALL, IF, IMPORT, LIST, LOCAL, MEMBER, MODULE, NULL, NUMBER, OBJECT, PARAM, PROCEDURE, PROXYVAL, QUASI, QUOTED, REF, RETURN, STRING, SWITCH, SYMBOL, TASK, TASKCALL, TEMPVAR, THROW, TOPLEVEL, TR, TRY, UNIT, UNQUOTE, UNQUOTESPLICING, VAR, WHILE, errorlet, esnode, loglet, _hashCode,
    __slice = [].slice,
  ...
  AST.register(SYMBOL = (function(_super) {
    ...
    SYMBOL.prototype.nested = function() {
      return new this.constructor(this.value, this.suffix === void 0 ? 1 : this.suffix + 1);
    };
  ...
  AST.register(MEMBER = (function(_super) {
    ...
    MEMBER.prototype.toESNode = function() {
      var head, key;
      head = this.head.toESNode();
      key = this.key.type() === 'symbol' ? esnode.literal(this.key.value) : this.key.toESNode();
    ...
    DEFINE.prototype.toESNode = function() {
      ...
      name = (function() {
        switch (this.name.type()) {
          case 'ref':
            return esnode.literal(this.name.name.value);
          case 'symbol':
            return esnode.literal(this.name.value);
          default:
            throw new Error("AST.define.toESNode:unknown_name_type: " + this.name);
        }
      }).call(this);
      value = esnode.funcall(esnode.member(esnode.identifier('_module'), esnode.identifier('define')), [name, this.value.toESNode()]);
      id = (function() {
        switch (this.name.type()) {
          case 'ref':
            return this.name.normalName().toESNode();
          case 'symbol':
  ...
  AST.register(REF = (function(_super) {
    ...
    REF.prototype.toESNode = function() {
      if (this.value.type() === 'proxyval') {
        return this.value.toESNode();
      } else if (this.isDefine) {
        return esnode.funcall(esnode.member(esnode.identifier('_module'), esnode.identifier('get')), [esnode.literal(this.name.value)]);
      } else {
        return this.name.toESNode();
      }
    };
  ...
  AST.register(PROXYVAL = (function(_super) {
    ...
    PROXYVAL.prototype.toESNode = function() {
      if (typeof this.compiler === 'function' || this.compiler instanceof Function) {
        return this.compiler();
      } else if (this.compiler instanceof AST) {
        return this.compiler.toESNode();
      } else {
        return this.name;
      }
    };
    ...
    PROXYVAL.prototype.selfESNode = function() {
      return this.baseSelfESNode(esnode.literal(this.name), this.esnode.literal(this.compiler));
    };
  ...
  AST.register(TOPLEVEL = (function(_super) {
    ...
    function TOPLEVEL(body) {
      ...
      this.moduleParam = AST.param(AST.symbol('_module'));
    ...
    TOPLEVEL.prototype.toString = function() {
      return "{TOPLEVEL " + this.body + "}";
    };

    TOPLEVEL.prototype.toESNode = function() {
      var imp;
      return esnode.funcall(esnode.member(esnode.identifier('_rt'), esnode.identifier('toplevel')), [
        esnode.array((function() {
          var _i, _len, _ref, _results;
          _ref = this.imports;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            imp = _ref[_i];
            _results.push(imp.specESNode());
          }
          return _results;
        }).call(this)), esnode["function"](null, [this.moduleParam.toESNode()].concat((function() {
          var _i, _len, _ref, _results;
          _ref = this.imports;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            imp = _ref[_i];
            _results.push(imp.idESNode());
          }
          return _results;
        }).call(this)).concat([this.callbackParam.toESNode()]), this.body.toESNode())
      ]);
    };

    TOPLEVEL.prototype.selfESNode = function() {
      return this.baseSelfESNode(this.body.selfESNode());
    };
  ...
  AST.register(MODULE = (function(_super) {
    ...
    MODULE.prototype.toESNode = function() {
      var imp;
      return esnode.funcall(esnode.member(esnode.identifier('_rt'), esnode.identifier('module')), [
        this.spec.toESNode(), esnode.array((function() {
          var _i, _len, _ref, _results;
          _ref = this.imports;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
  ...
  AST.register(BINDING = (function(_super) {
    ...
    BINDING.prototype.toString = function() {
      if (this.as) {
        return "{AS " + this.spec + " " + this.as + "}";
      } else {
        return "{" + this.spec + "}";
      }
    };

    BINDING.prototype.toESNode = function() {};
  ...
  AST.register(IMPORT = (function(_super) {
    ...
    IMPORT.prototype.toString = function() {
      return "{IMPORT " + this.spec + "} " + this.bindings;
    };

    IMPORT.prototype.specESNode = function() {
      return this.spec.toESNode();
    };
    ...
    IMPORT.prototype.define = function(binding) {
      return AST.define(binding.as, AST.funcall(AST.member(this.idParam.ref(), AST.symbol('get')), [AST.string(binding.spec.value)]));
    };

    IMPORT.prototype.proxy = function(binding) {
      return AST.proxyval(binding.spec, AST.funcall(AST.member(AST.symbol('_module'), AST.symbol('get')), [AST.string(binding.spec.value)]));
    };

    IMPORT.prototype.idESNode = function() {
      return this.idParam.toESNode();
    };

    IMPORT.prototype.toESNode = function() {
      var binding;
      return esnode.declare.apply(esnode, ['var'].concat(__slice.call((function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          binding = _ref[_i];
          _results.push(this.bindingESNode(binding));
        }
        return _results;
      }).call(this))));
    };
    ...
    IMPORT.prototype.bindingESNode = function(binding) {
      var res;
      res = [binding.as.toESNode(), esnode.member(this.idParam.toESNode(), binding.spec.toESNode())];
  ...
  AST.register(EXPORT = (function(_super) {
    ...
    EXPORT.prototype.toESNode = function() {
