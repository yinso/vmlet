...
(function() {
  var ARRAY, AST, BINARY, BLOCK, BOOL, CATCH, DEFINE, FINALLY, FUNCALL, IF, LIST, MEMBER, NULL, NUMBER, OBJECT, PARAM, PROC, QUASI, QUOTED, STRING, SYMBOL, THROW, TRY, UNQUOTE, UNQUOTESPLICING, errorlet, loglet,
    __slice = [].slice,
  ...
  AST.register(BLOCK = (function(_super) {
    ...
    BLOCK.prototype._equals = function(v) {
      var i, res, val, _i, _len, _ref;
      if (v.val.length === this.val.length) {
        _ref = v.val;
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          val = _ref[i];
          res = this.val[i].equals(val);
  ...
  AST.register(PROC = (function(_super) {

    __extends(PROC, _super);

    PROC.type = 'proc';

    function PROC(name, params, body, returns) {
      this.name = name;
      this.params = params;
      this.body = body;
    ...
    PROC.prototype._equals = function(v) {
    ...
    PROC.prototype.toString = function() {
      return "{PROC " + this.name + " " + this.params + " " + this.body + " " + this.returns + "}";
    };
