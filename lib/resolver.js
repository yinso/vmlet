...
(function() {
  var ANF, AST, SymbolTable, T, errorlet, get, loglet, makeProc, register, tr, transform, transformArray, transformBinary, transformBlock, transformCatch, transformDefine, transformDefineName, transformDefineVal, transformExport, transformFinally, transformFuncall, transformIdentifier, transformIf, transformImport, transformLet, transformMember, transformObject, transformParam, transformScalar, transformTaskcall, transformThrow, transformTopLevel, transformTry, _transTypes, _transform;
  ...
  transform = function(ast, env) {
    ...
    switch (ast.type()) {
      ...
      case 'module':
        ...
        return T.transform(ast.clone(AST["return"](anf)));
      default:
        resolved = _transform(ast, env);
        anf = ANF.transform(resolved, env);
        return T.transform(AST["return"](anf));
    }
  };
  ...
  transformBlock = function(ast, env) {
    ...
    items = (function() {
      ...
      for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
        item = _ref1[i];
        if (item.type() === 'define') {
          _results.push(transformDefineVal(item, env));
        } else {
  ...
  transformDefineVal = function(ast, env) {
    var ref, res;
  ...
  transformDefine = tr.trace('resolver.define', function(ast, env) {
    transformDefineName(ast, env);
    return transformDefineVal(ast, env);
  });
  ...
  transformImport = function(ast, env) {
  ...
  transformExport = function(ast, env) {
