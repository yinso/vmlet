...
(function() {
  var AST, Environment, assign, errorlet, get, makeProc, normalize, normalizeBlock, override, register, tr, transform, transformArray, transformBinary, transformBlock, transformCatch, transformDefine, transformExport, transformFinally, transformFuncall, transformIdentifier, transformIf, transformImport, transformLet, transformLocal, transformMember, transformObject, transformParam, transformProc, transformScalar, transformTaskcall, transformThrow, transformTry, _transTypes, _transform, _transformInner;
  ...
  transformDefine = function(ast, env, block) {
    var exp, i, res, _i, _len, _ref, _results;
    ...
    if (res.type() === 'block') {
      ...
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        ...
        if (i < res.items.length - 1) {
          ...
          _results.push(block.push(AST.define(ast.name, exp)));
        }
      }
      return _results;
    } else {
      return block.push(AST.define(ast.name, res));
    }
  };

  register(AST.get('define'), transformDefine);

  transformLocal = function(ast, env, block) {
    var cloned, exp, i, res, _i, _len, _ref, _results;
    res = ast.value ? transform(ast.value, env) : ast.value;
    if (res.type() === 'block') {
      _ref = res.items;
      _results = [];
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        ...
        if (i < res.items.length - 1) {
          ...
          _results.push(block.push(cloned));
        }
      }
      return _results;
    } else {
  ...
  transformImport = function(ast, env, block) {
    var define, _i, _len, _ref;
    _ref = ast.defines();
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      define = _ref[_i];
      block.push(define);
  ...
  transformExport = function(ast, env, block) {
    var binding, ref, _i, _len, _ref;
    _ref = ast.bindings;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      binding = _ref[_i];
  ...
  transformLet = function(ast, env, block) {
    var body, define, defines, exp, newEnv, res, _i, _j, _len, _len1, _ref, _ref1, _results;
    newEnv = new Environment(env);
    defines = [];
    _ref = ast.defines;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
